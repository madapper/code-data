...
extension Clonable where Self: NSObject {
    var clone: Self {
        let clone = Self.init()
        for property in derivedProperties.updatable {
...
extension NSObject {
    ...
    static var derivedProperties: [objc_property_t] {
        ...
        if let parent = class_getSuperclass(classForKeyedArchiver) as? NSObject.Type {
            properties.append(contentsOf: parent.derivedProperties)
        }
        return properties
    }
    
    var derivedProperties: [objc_property_t] {
        return type(of: self).derivedProperties
    }
